...
def organization(db, admin):
    return Organization.objects.create(name="Test Organization", owner=admin)
...
@pytest.fixture
def orderer(db, organization):
    return Customer.objects.create(name="orderer",organization=organization)
...
@pytest.mark.django_db
def test_create_product(regular_client,organization):
...
def test_accept_pending_order_by_owner(admin, order_model, regular_client):
    ...
    assert response.data.get("assigned_to").get("email") == admin.email
...
def test_accept_pending_order_by_staff(staff, order_model, regular_client, staff_token):
    ...
    assert response.status_code == 200
    assert response.data.get("status") == "ACCEPTED"
    assert response.data.get("assigned_to").get("email") == staff.email
